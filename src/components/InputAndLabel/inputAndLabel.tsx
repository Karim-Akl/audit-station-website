...
export default function InputAndLabel({
  ...
  const [typeInput, setTypeInput] = useState<React.ComponentProps<'input'>['type']>('');
    ...
    >
      ...
      <div className='relative w-[100%]'>
        <input
          ...
          className={`${
            icon ? 'ps-14' : ''
          } text-[#333] shadow rounded-md border-2 outline-none h-[55px] mb-1 ${
            error ? 'border-red-500' : 'border-gray-200 focus:border-[#22B9DD]'
          } w-full px-6 py-3`}
        />
        {icon && (
          <div
            className={`absolute  ${
              t('language') === 'ar' ? 'right-2' : 'left-2'
